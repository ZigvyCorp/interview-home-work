  ...
  return (
    <>
      <div className="Header">
        <div className="Header1">Logo</div>
        <div className="Header2">Blogs</div>
        <div className="Header3">Pham Quoc Thinh</div>
      </div>
      {posts.map((post, index) => {
        return (
          <div key={index} className="App">
            <div className="Main">
              <h1 className="Main1">{post.title}</h1>

              {users.map((user, index) => {
                return (
                  <div key={index} className="Main2_1">
                    {user.id === post.owner ? (
                      <div className="left">
                        <div>
                          <div>Author: {user.username}</div>
                          <div>
                            Create at: {Date(user.created_at).slice(3, 15)}
                          </div>
                        </div>
                          ...
                          <>
                            ...
                            <div>
                              <div className="Reply_name">
                                ...
                                <div>
                                  {Date(comment.created_at).slice(3, 15)}
                                </div>
                              </div>
                              <div className="content">{comment.content}</div>
                              <div className="reply">Reply to</div>
                            </div>
                          </>
                        ) : (
                          ""
