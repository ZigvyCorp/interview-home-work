import React, { useEffect, useState } from "react";
// import users from "../../common/data/users.json";
import ReplyIcon from "@mui/icons-material/Reply";
import moment from "moment/moment";
export default function Comment({ comment }) {
  const [isActiveReply, setIsActiveReply] = useState(false);
  const [text, setText] = useState("");
  const [listReply, setListReply] = useState([]);
  const [dateReply, setDateReply] = useState(new Date(Date.now()));
  const dateFormat = moment(dateReply).format("YY-MM-DD, h:mm ");
  let user = comment.owner;
  const toggleReply = () => {
    setIsActiveReply(!isActiveReply);
  };
  useEffect(() => {}, [comment]);

  const handleSubmit = () => {
    if (text !== "") {
      setListReply([...listReply, text]);
      setText("");
    }
  };

  const dateComment = moment(comment.createdAt).format("YY-MM-DD, h:mm ");
  return (
    <div className="flex h-auto mx-5 mb-3" key={comment}>
      <div className="flex-none h-20 w-20 ml-5 ">
        <div className="flex  items-center mt-2">
          <img
            className="inline-block h-10 w-10 rounded-full ring-2 ring-white"
            src="data:image/png;base64,"
            alt=""
          />
        </div>
      </div>

      <div className="flex-1 w-64 ml-5 mt-3">
        <div className="flex ">
          <h2 className="text-gray-500 text-sm ">{user.username}</h2>
          <h3 className="text-gray-400 ml-4 text-sm">{dateComment}</h3>
        </div>
        <div className="flex ">
          <h2 className="text-gray-800 text-base text-justify">
            {comment.content}
          </h2>
        </div>

        <div className="w-full mt-[-40px] flex" id="listreply">
          <ul>
            {listReply.map((item) => (
              <div className="h-auto ">
                <ReplyIcon style={{ fontSize: 30, marginRight: 10 }} />{" "}
                {dateFormat}
                <li>{item}</li>
              </div>
            ))}
          </ul>
        </div>
        <button
          className="flex mt-5 text-gray-500 text-sm "
          onClick={toggleReply}
        >
          Reply to
        </button>

        {isActiveReply ? (
          <div className="flex mt-2">
            <input
              class="shadow appearance-none border border-cyan-500 rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
              id="comment"
              type="text"
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="Comment here"
            />
            <button
              className="w-20 h-10 ml-2 rounded-md border-cyan-500 border-[1px]"
              onClick={handleSubmit}
            >
              Send
            </button>
          </div>
        ) : (
          ""
        )}
      </div>
    </div>
  );
}
