...
import { ACTION, NEXT_ROUTER_PREFETCH, RSC, RSC_CONTENT_TYPE_HEADER, RSC_VARY_HEADER } from "../client/components/app-router-headers";
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, turboNextBuildRoot = null, buildMode) {
    ...
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const useBuildWorker = config.experimental.webpackBuildWorker || !config.webpack;
            if (config.webpack && config.experimental.webpackBuildWorker === undefined) {
                Log.warn('Custom webpack configuration is detected. When using a custom webpack configuration, the Webpack build worker is disabled by default. To force enable it, set the "experimental.webpackBuildWorker" option to "true". Read more: https://nextjs.org/docs/messages/webpack-build-worker-opt-out');
            }
            if (!isGenerate) {
                if (isCompile && useBuildWorker) {
                    let durationInSeconds = 0;
                    await webpackBuild(true, [
                        "server"
                    ]).then((res)=>{
                    ...
                    const { duration: webpackBuildDuration, ...rest } = turboNextBuild ? await turbopackBuild() : await webpackBuild(false);
            ...
            const { ipcPort: incrementalCacheIpcPort, ipcValidationKey: incrementalCacheIpcValidationKey } = await initializeIncrementalCache({
                fs: nodeFs,
                dev: false,
                                ...
                                try {
                                    ...
                                    if (pageType === "app" && originalAppPath) {
                                        ...
                                        if (isEdgeRuntime(pageRuntime)) {
                                            ...
                                            if (!isStatic && !isAppRouteRoute(originalAppPath) && !isDynamicRoute(originalAppPath)) {
